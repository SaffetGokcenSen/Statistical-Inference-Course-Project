...
library(dplyr)
data_set <- distinct(ToothGrowth)
summary(data_set)
str(data_set)
```

The unique dose values are `r unique(data_set$dose)`. To have a quick overview 
...
group_05 <- filter(data_set, dose==0.5)
group_1 <- filter(data_set, dose==1.0)
len_05 <- group_05$len
len_1 <- group_1$len
...
group_2 <- filter(data_set, dose==2.0)
len_2 <- group_2$len
